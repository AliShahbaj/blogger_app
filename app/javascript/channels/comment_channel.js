...
consumer.subscriptions.create("CommentChannel", {
  ...
  received(data) {
    ...
    if(comment.parent_id){
      ...
      $(`.post_${comment.post_id}`).append(`<div id="comment_${comment.id}"> <p class="m-0"><span>${comment.body} <a data-remote="true" href="/posts/7/comments/new?parent_id=${comment.id}" class="float-right">Reply</a></span></p> <em>${comment.email} - ${comment.time}</em><div id="replies_${comment.id}"></div> </div>`);
